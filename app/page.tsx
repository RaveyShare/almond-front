...
import { Brain, Mic, Loader2, Calendar, Star, ArrowRight, Eye, Share2, ListTodo, PenTool } from "lucide-react"
...
import type { MemoryItem, MemoryAids, Mnemonic, SensoryAssociation as SensoryAssociationType, UnifiedItem } from "@/lib/types"
...
export default function Home() {
  const [inputType, setInputType] = useState<"memory" | "task">("memory")
  const [inputValue, setInputValue] = useState("")
  const [isLoading, setIsLoading] = useState(false)
  const [isAuthenticated, setIsAuthenticated] = useState(false)
  const [currentUser, setCurrentUser] = useState(authManager.getCurrentUser())
  const [memoryItems, setMemoryItems] = useState<UnifiedItem[]>([])
  ...
  const handleMemoryItemClick = (item: UnifiedItem) => {
    if (item.type === 'memory') {
      ...
      <section className="relative flex min-h-screen items-center justify-center overflow-hidden pt-16">
        ...
        <div className="container relative z-[3] px-4 text-center">
          ...
          >
            ...
            <p className="mx-auto max-w-2xl text-white/80 sm:text-xl">它不是帮你整理过去，而是陪你走向更好的自己。</p>
