...
const base = process.env.NEXT_PUBLIC_FRONT_AUTH_URL || (process.env.NEXT_PUBLIC_USER_CENTER_URL?.replace(/\/$/, "") || "https://user-center.ravey.site")
...
async function handle(req: NextRequest, params: { path: string[] }) {
  const dest = `${base}/front/users/${(params.path || []).join("/")}`
